...
/**
 * @brief Reads the calibrated turbidity value from the analog turbidity sensor.
 * Uses a hardcoded 2-point linear map (clear→turbid). See TURB_V_* in Sensors.cpp.
 * @return Calibrated turbidity value in NTU (float).
 */
float readTurbidityNTU();

/**
 * @brief Computes a qualitative water clarity label from an NTU value.
 * @param ntuValue The NTU value from the turbidity sensor.
 * @return "Jernih", "Keruh", or "Sangat Keruh".
 */
